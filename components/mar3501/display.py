...
from . import mar3501_ns

MAR3501 = mar3501_ns.class_(
    "Mar3501", cg.PollingComponent, display.DisplayBuffer
)

def validate_mar3501(config):
   return config
...
CONFIG_SCHEMA = cv.All(
    display.FULL_DISPLAY_SCHEMA.extend(
        {
            cv.GenerateID(): cv.declare_id(MAR3501),
...
async def to_code(config):
    if CONF_USER_BUILD_FLAGS in config:
        _LOGGER.warning("Setting custom TFT_eSPI PlatformIO build_flags for MAR3501 as follows:")
        for key in config[CONF_USER_BUILD_FLAGS]:
            cg.add_build_flag("-D" + key)
            _LOGGER.warning("-D" + key)
    else:
        # Add platformio build_flags for the correct TFT_eSPI settings for the MAR3501
